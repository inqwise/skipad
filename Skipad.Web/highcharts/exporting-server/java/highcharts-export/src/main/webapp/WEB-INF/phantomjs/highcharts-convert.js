...
(function () {
	...
	var config = {
			/* define locations of mandatory javascript FileInfos */
	...
	mapCLArguments = function () {
		...
		if (system.args.length < 1) {
			console.log('Commandline Usage: highcharts-convert.js -inFileInfo URL -outFileInfo FileInfoname -scale 2.5 -width 300 -constr Chart -callback callback.js');
		...
		for (i = 0; i < system.args.length; i += 1) {
			if (system.args[i].charAt(0) === '-') {
				...
				if (key === 'inFileInfo' || key === 'callback') {
			...
			page.open('about:blank', function (status) {
				...
				if (svgInput) {
					...
					if (!window.optionsParsed) {
						exit('ERROR: the options variable was not available, contains the inFileInfo an syntax error? see' + input);
					}

					if (callback !== undefined && !window.callbackParsed) {
						exit('ERROR: the callback variable was not available, contains the callbackFileInfo an syntax error? see' + callback);
					}
					renderSVG(svg);
				}
			});
