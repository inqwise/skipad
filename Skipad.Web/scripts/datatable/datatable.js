    ...
    $.fn.extend({
        dataTable: function (o) {
            ...
            var V = function (p) {
                ...
                var h = 0;
                var k = 0;
                var f = u.length / options.pagingStart;
                for (var w = 0; w < f; w++) {
                    k++
                }
                n = k;
                Q.empty();
                for (var i = e; i < u.length; i++) {
            ...
            var b = function (p) {
                ...
                if (u.length > 0) {
                    V(p);
                    if (options.pagingStart > 0 && u.length >= options.pagingStart) {
                        if (u.length <= options.pagingStart) {
                            F.show();
                            //H.hide();
                            I.show();
                            J.hide()
                    ...
                    } else if (options.pagingStart >= u.length) {
                        F.show();
                        //H.hide();
                        I.show();
                        J.hide()
                    ...
                    Q.empty();
                    var colspan = options.tableColumns.length + ((options.selectable ? 1 : 0) + (options.expandable ? 1 : 0));
                    $("<tr class=\"nop\">" + "<td colspan=\"" + colspan + "\"><b>" + options.messages.noData + "</b></td>" + "</tr>").appendTo(Q);
            ...
            if (options.tableColumns != null) {
                ...
                for (var i = 0; i < options.tableColumns.length; i++) {
                    var h = $("<th " + ((i == options.tableColumns.length - 1) ? "class=\"last-item\"" : "") + ((i == 0) ? (options.selectable ? "" : (options.expandable ? "" : "class=\"first-item\"")) : "") + "><div><span>" + options.tableColumns[i].label + "</span>" + (options.tableColumns[i].help != undefined ? "<span class=\"help\" title=\"" + options.tableColumns[i].help + "\"></span>" : "") + "</div></th>").appendTo(Y);
                    
                    // help tooltip
                    if(jQuery.fn.tipsy != undefined) {
                    	h.find(".help").tipsy({
        					gravity: 'se', 
        					title: function() { 
                ...
                var Z = function (v) {
					...
					if(options.groupBy != null) {
						...
						if (a.length > 0) {
							...
							Q.empty();
							var colspan = options.tableColumns.length + ((options.selectable ? 1 : 0) + (options.expandable ? 1 : 0));
							$("<tr class=\"nom\">" + "<td colspan=\"" + colspan + "\"><b>" + options.messages.noRecords + "</b></td>" + "</tr>").appendTo(Q);
							A.find("#" + B + "_label_results").html("<b>0</b> - <b>0</b> of <b>0</b>");
